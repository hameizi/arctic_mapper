...
public interface DerbyContainerMetadataMapper extends ContainerMetadataMapper {
  ...
  @Insert("MERGE INTO " +
      ...
      "ON APP.container_metadata.container_name = #{container.containerName} " +
      "WHEN matched THEN update set container_type =#{container.containerType}, properties =#{container.properties, " +
      "typeHandler=com.netease.arctic.ams.server.mybatis.Map2StringConverter} " +
      "WHEN NOT MATCHED THEN INSERT (container_name,container_type,properties) VALUES (#{container.containerName}, " +
      "#{container.containerType}, #{container.properties, typeHandler=com.netease.arctic.ams.server.mybatis" +
      ".Map2StringConverter})")
