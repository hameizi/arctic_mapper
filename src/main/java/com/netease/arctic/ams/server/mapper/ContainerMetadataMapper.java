...
public interface ContainerMetadataMapper {
  ...
  @Select("select container_name, container_type, properties from " + TABLE_NAME)
  @Results({
  ...
  @Select("select container_name, container_type, properties from " + TABLE_NAME + " where container_name = #{name}")
  @Results({
      @Result(property = "containerName", column = "container_name"),
      @Result(property = "containerType", column = "container_type"),
      @Result(property = "properties", column = "properties", typeHandler = Map2StringConverter.class)
  ...
  @Select("select container_type from " + TABLE_NAME + " where container_name = #{name}")
  String getType(String name);

  @Insert("replace into " + TABLE_NAME +
      " (container_name, container_type, properties) values (#{container.containerName}, #{container.containerType}, " +
